...
async def main():
    ...
    webchatURL = 'https://cx.filum.ai/conversation-channels/webchat/3184'
    workspace = 'thien+dance'

    for i in range(2):
        agent = Agent(
            task=(
                'Important : I am a UI Automation Tester sending message in vietnamese to webchat of Filum company by using Playwright. to verify response \n'
                'Remember: wait for response message before sending the next message\n'
                '1. Open browser, check if login is required, if yes, then login to "cx.filum.ai" with username "nghia.huynh@filumai.com" and password "Nghiahuynh832@", if not, then continue to step 2\n'
                f'2. Select workspace in the left of avatar icon, then select {workspace} workspace\n'
                f'3. Open new tabs navigate to website {webchatURL} \n'
                '4. Open webchat in bottom right corner, then send a message to Filum webchat, the message is "Hello, this is a test message from Playwright" \n'
                '5. Wait for response message from Filum webchat, then record the sent message and response message \n'
                '6. Send message "cho tui xin giá mấy chiếc xe bên mình đi" to Filum webchat" \n'
                '7. Wait for response message from Filum webchat, then record the sent message and response message \n'
                '8. Send message "xe cùi bắp mà cũng không có, cáu thật" to Filum webchat\n'
                '9. Wait for response message from Filum webchat, then record the sent message and response message \n'
                '10. Click on avatar icon in the top right corner, then click on "Đăng xuất" or "Log out" button to logout\n'
                '11. Show me the sent messages and response messages when your task is done.\n'
            ),
            llm=ChatGoogle(
